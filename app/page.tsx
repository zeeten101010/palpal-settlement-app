...
function getMonthProjection(summary: Summary | null, month: string) {
  ...
  const target80m = 80000000;
  const target100m = 100000000;
  const requiredDaily80m = remainingDays > 0 ? Math.max(0, Math.round((target80m - revenue) / remainingDays)) : 0;
  const requiredDaily100m = remainingDays > 0 ? Math.max(0, Math.round((target100m - revenue) / remainingDays)) : 0;

  const status =
    projectedRevenue >= target100m ? "1억 이상 페이스" :
    projectedRevenue >= target80m ? "8천만 이상 페이스" :
    projectedRevenue >= 70000000 ? "7천만 이상 페이스" :
    "매출 보강 필요";
          ...
          <div className="sales-projection-card">
            ...
            <div className="sales-projection-comment">
              ...
              {salesProjection.isCurrentMonth && salesProjection.remainingDays > 0 && (
                <>
                  <br />
                  8천만 원을 맞추려면 남은 {salesProjection.remainingDays}일 동안 하루 평균 {formatWon(salesProjection.requiredDaily80m)},
                  1억 원을 맞추려면 하루 평균 {formatWon(salesProjection.requiredDaily100m)}가 필요합니다.
                </>
              )}
            </div>
          </div>
