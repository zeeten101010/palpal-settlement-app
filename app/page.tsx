        ...
        <section className="card">
          <div className="cash-hero">
            <div>
              ...
              <p>자금일보는 월결산 손익과 별개로 저장됩니다. 출금예정 입력 시 출금예정 합계, 예상잔액, 미지급 거래처 요약만 자동 변경됩니다.</p>
            </div>
            <div className="actions no-print">
          ...
          <form className="cash-form no-print" onSubmit={saveCashItem}>
            <div className="cash-form-title">{cashForm.id ? "자금일보 항목 수정" : "자금일보 항목 입력"}</div>
            <div className="grid cash-form-grid">
              <div className="field">
                <label>항목구분</label>
                <select className="select" value={cashForm.item_type} onChange={(e) => updateCashForm("item_type", e.target.value as CashItemType)}>
                  <option value="account">자금현황</option>
                  <option value="out_done">출금완료</option>
                  <option value="out_scheduled">출금예정</option>
                </select>
              </div>

              <div className="field">
                <label>{cashForm.item_type === "account" ? "기준일" : cashForm.item_type === "out_done" ? "출금일" : "예정일"}</label>
                <input className="input" type="date" value={cashForm.record_date} onChange={(e) => updateCashForm("record_date", e.target.value)} />
              </div>

              <div className="field">
                <label>소속</label>
                <select className="select" value={cashForm.group_name} onChange={(e) => updateCashForm("group_name", e.target.value)}>
                  {cashGroups.map((group) => <option key={group} value={group}>{group}</option>)}
                </select>
              </div>

              {cashForm.item_type === "account" ? (
                <>
                  <div className="field">
                    <label>금융기관</label>
                    <input className="input" value={cashForm.bank} onChange={(e) => updateCashForm("bank", e.target.value)} placeholder="우리은행" />
                  </div>
                  <div className="field">
                    <label>계좌명</label>
                    <input className="input" value={cashForm.account_name} onChange={(e) => updateCashForm("account_name", e.target.value)} placeholder="팔팔 운영계좌" />
                  </div>
                  <div className="field">
                    <label>계좌번호</label>
                    <input className="input" value={cashForm.account_number} onChange={(e) => updateCashForm("account_number", e.target.value)} placeholder="선택 입력" />
                  </div>
                </>
              ) : (
                <>
                  <div className="field">
                    <label>구분</label>
                    <select className="select" value={cashForm.category} onChange={(e) => updateCashForm("category", e.target.value)}>
                      {cashCategories.map((category) => <option key={category} value={category}>{category}</option>)}
                    </select>
                  </div>
                  <div className="field">
                    <label>거래처</label>
                    <input className="input" value={cashForm.vendor} onChange={(e) => updateCashForm("vendor", e.target.value)} placeholder="단군" />
                  </div>
                  <div className="field">
                    <label>결제계좌</label>
                    <input className="input" value={cashForm.account_name} onChange={(e) => updateCashForm("account_name", e.target.value)} placeholder="팔팔 / 프워" />
                  </div>
                </>
              )}

              <div className="field">
                <label>{cashForm.item_type === "account" ? "잔액" : "금액"}</label>
                <input
                  className="input"
                  value={cashForm.amount ? Number(normalizeAmount(cashForm.amount)).toLocaleString("ko-KR") : ""}
                  onChange={(e) => updateCashForm("amount", e.target.value)}
                  placeholder="0"
                />
              </div>
              ...
              {cashForm.item_type !== "account" && (
                <div className="field">
                  <label>상태</label>
                  <select className="select" value={cashForm.status} onChange={(e) => updateCashForm("status", e.target.value)}>
                    {cashStatuses.map((status) => <option key={status} value={status}>{status}</option>)}
                  </select>
                </div>
              )}
              ...
              <div className="field">
                <label>메모</label>
                <input className="input" value={cashForm.memo} onChange={(e) => updateCashForm("memo", e.target.value)} placeholder="입고일/비고" />
              </div>
              ...
              {cashForm.item_type === "account" && (
                <div className="field checkbox-field">
                  ...
                  <label className="cash-checkbox">
                    <input type="checkbox" checked={cashForm.exclude_from_cash} onChange={(e) => updateCashForm("exclude_from_cash", e.target.checked)} />
                    보유자금 합계에서 제외
                  </label>
                </div>
              )}
            ...
            <div className="cash-form-actions">
              <button className="btn btn-green" type="submit" disabled={cashSaving}>{cashSaving ? "저장 중..." : cashForm.id ? "수정 저장" : "+ 자금일보 저장"}</button>
              {cashForm.id && <button className="btn btn-ghost" type="button" onClick={() => setCashForm(blankCashForm(month))}>수정 취소</button>}
            </div>
          </form>
          ...
          <div className="cash-layout">
            <div className="cash-panel">
              ...
              <CashTable
                columns={["소속", "계좌명", "잔액", "메모", "관리"]}
                rows={cashReport.accounts}
                empty="자금현황 항목이 없습니다."
                renderRow={(item) => (
                  <tr key={item.id}>
                    ...
                    <td>
                      ...
                      <small>{item.bank || ""} {item.account_number || ""}</small>
                    </td>
                    <td className="num">{formatWon(item.amount)}</td>
                    <td>{item.exclude_from_cash ? "보유자금 제외" : item.memo || ""}</td>
                    <td className="table-actions no-print">
                      <button onClick={() => editCashItem(item)}>수정</button>
                      <button className="danger" onClick={() => deleteCashItem(item.id)}>삭제</button>
                    </td>
                  </tr>
                )}
              />
            ...
            <div className="cash-panel">
              <div className="cash-panel-head">
                <h3>금일 출금완료</h3>
                <span>{cashDate} 합계 {formatWon(cashReport.paidTodayTotal)}</span>
              </div>
              <CashTable
                columns={["출금일", "거래처", "금액", "계좌", "관리"]}
                rows={cashReport.paidToday}
                empty="해당 기준일의 출금완료 항목이 없습니다."
                renderRow={(item) => (
                  <tr key={item.id}>
                    <td>{item.record_date}</td>
                    <td><strong>{item.vendor || "-"}</strong><small>{item.category || ""}</small></td>
                    <td className="num red">{formatWon(item.amount)}</td>
                    <td>{item.account_name || item.group_name}</td>
                    <td className="table-actions no-print">
                      <button onClick={() => editCashItem(item)}>수정</button>
                      <button className="danger" onClick={() => deleteCashItem(item.id)}>삭제</button>
                    </td>
            ...
            <div className="cash-panel">
              ...
              <CashTable
                columns={["예정일", "거래처", "미지급잔액", "상태", "관리"]}
                rows={cashReport.scheduled}
                empty="출금예정 항목이 없습니다."
                renderRow={(item) => (
                  <tr key={item.id}>
                    <td>{item.record_date}</td>
                    <td><strong>{item.vendor || "-"}</strong><small>{item.memo || item.category || ""}</small></td>
                    <td className="num amber">{formatWon(item.amount)}</td>
                    <td>{item.status || "예정"}</td>
                    <td className="table-actions no-print">
                      <button onClick={() => editCashItem(item)}>수정</button>
                      <button className="danger" onClick={() => deleteCashItem(item.id)}>삭제</button>
                    </td>
