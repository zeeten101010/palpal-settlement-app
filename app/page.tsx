...
type Tab = "input" | "list" | "accounts" | "summary" | "analysis" | "franchise" | "cash";
...
const targetSalesLow = 60000000;
const targetSalesHigh = 65000000;
...
const costCategories = ["육류", "식자재", "음료", "주류", "기타"];

const blankCostInventoryRows = (): CostInventoryFormRow[] =>
...
const blankForm = (_month: string) => {
  const today = getTodayDate();

  return {
    id: "",
    transaction_date: today,
    settlement_month: toSettlementMonth(today),
...
function getMonthProjection(summary: Summary | null, month: string) {
  ...
  const dailyRevenue = revenue / Math.max(elapsedDays, 1);
  const projectedRevenue = isCurrentMonth ? Math.round(dailyRevenue * daysInMonth) : revenue;
  const profitRate = calcRate(profit, revenue);
  const projectedProfit = revenue > 0 ? Math.round(projectedRevenue * (profit / revenue)) : 0;

  const requiredDailyTargetLow = remainingDays > 0 ? Math.max(0, Math.round((targetSalesLow - revenue) / remainingDays)) : 0;
  const requiredDailyTargetHigh = remainingDays > 0 ? Math.max(0, Math.round((targetSalesHigh - revenue) / remainingDays)) : 0;

  const status =
    projectedRevenue >= targetSalesHigh ? "목표 상단 초과 페이스" :
    projectedRevenue >= targetSalesLow ? "목표권 진입 페이스" :
    projectedRevenue >= targetSalesLow * 0.9 ? "목표 근접 페이스" :
    "매출 보강 필요";

  const comment = revenue <= 0
    ? "현재 입력된 매출이 없어 월 매출 예상치를 계산할 수 없습니다."
    : isCurrentMonth
      ? `현재 일평균 매출은 ${formatWon(Math.round(dailyRevenue))}입니다. 이 페이스면 월매출은 약 ${formatWon(projectedRevenue)}로 예상됩니다.`
      : `선택한 월은 종료된 월로 보고 실제 매출 ${formatWon(revenue)} 기준으로 표시합니다.`;
  ...
  return {
    ...
    targetSalesLow,
    targetSalesHigh,
    requiredDailyTargetLow,
    requiredDailyTargetHigh,
    status,
    comment,
    isCurrentMonth
  };
...
function buildMonthlyAnalysis(summary: Summary | null, prevSummary: Summary | null, transactions: Transaction[]) {
  ...
  const targetRevenueLow = targetSalesLow;
  const targetRevenueHigh = targetSalesHigh;
  const targetProfitLow = Math.round(targetRevenueLow * targetProfitRate);
  const targetProfitHigh = Math.round(targetRevenueHigh * targetProfitRate);

  const nextMonth = [
    `다음달 목표 매출: ${formatWon(targetRevenueLow)} ~ ${formatWon(targetRevenueHigh)}`,
    `다음달 목표 순이익: ${formatWon(targetProfitLow)} ~ ${formatWon(targetProfitHigh)} 이상`,
    `목표 일평균 매출: ${formatWon(Math.round(targetRevenueLow / 30))} ~ ${formatWon(Math.round(targetRevenueHigh / 30))}`,
    `원가율 목표: 35~37% 이하`,
    `인건비율 목표: 22~24% 이하`,
    `광고비율 목표: 5% 이하`,
...
function getCostCategoryFromSummaryRow(row: Summary["rows"][number]) {
  const names = [
  ...
  if (names.includes("육류")) return "육류";
  if (names.includes("음료")) return "음료";
  if (names.includes("주류")) return "주류";
  if (names.includes("식자재")) return "식자재";
  return "기타";
}

function buildCostRateData(summary: Summary | null, costInventoryRows: CostInventoryFormRow[]) {
  const revenue = summary?.totalRevenue || 0;
  const rows = summary?.rows || [];

  const purchaseMap = new Map<string, number>();
  costCategories.forEach((category) => purchaseMap.set(category, 0));

  rows
    .filter((row) => row.transaction_type === "expense")
    .filter((row) => row.main_account_name === "매출원가")
    .filter((row) => !row.is_inventory_adjustment)
    .forEach((row) => {
      const category = getCostCategoryFromSummaryRow(row);
      purchaseMap.set(category, (purchaseMap.get(category) || 0) + row.total_amount);
    });

  const detailRows: CostRateRow[] = costCategories.map((category) => {
    const inventory = costInventoryRows.find((row) => row.category === category);
    const beginning = normalizeAmount(inventory?.beginning_inventory || "");
    const ending = normalizeAmount(inventory?.ending_inventory || "");
    const purchase = purchaseMap.get(category) || 0;
    const used = beginning + purchase - ending;
  ...
  const total = detailRows.reduce(
    (sum, row) => ({
      category: "합계",
      beginning: sum.beginning + row.beginning,
      purchase: sum.purchase + row.purchase,
      ending: sum.ending + row.ending,
      used: sum.used + row.used,
      rate: 0
    }),
    { category: "합계", beginning: 0, purchase: 0, ending: 0, used: 0, rate: 0 }
  );

  total.rate = calcRate(total.used, revenue);
...
function buildFranchisePnlData(summary: Summary | null, costRateData: ReturnType<typeof buildCostRateData>) {
  const rows = summary?.rows || [];
  const revenue = summary?.totalRevenue || 0;
  const adjustedCogs = costRateData.total.used;
  const purchaseCogs = costRateData.total.purchase;

  const grossProfit = revenue - adjustedCogs;
  const grossProfitRate = calcRate(grossProfit, revenue);
  ...
  rows
    ...
    .filter((row) => row.main_account_name !== "매출원가")
    .forEach((row) => {
      const label = row.main_account_name || "기타비용";
      map.set(label, (map.get(label) || 0) + row.total_amount);
    });

  const operatingExpenseRows = Array.from(map.entries())
    .map(([label, amount]) => ({
      label,
      amount,
  ...
  const operatingExpense = operatingExpenseRows.reduce((sum, row) => sum + row.amount, 0);
  const operatingProfit = grossProfit - operatingExpense;
  const operatingProfitRate = calcRate(operatingProfit, revenue);

  const pnlRows = [
    { label: "총매출", amount: revenue, rate: revenue ? 100 : 0, type: "revenue" },
    { label: "매출원가", amount: adjustedCogs, rate: calcRate(adjustedCogs, revenue), type: "cost" },
    { label: "매출총이익", amount: grossProfit, rate: grossProfitRate, type: "profit" },
    ...operatingExpenseRows.map((row) => ({ ...row, type: "expense" })),
    { label: "영업이익", amount: operatingProfit, rate: operatingProfitRate, type: "profit" }
  ];

  const paybackBase = operatingProfit > 0 ? Math.round(100000000 / operatingProfit * 10) / 10 : 0;
...
function buildReportData(summary: Summary | null, prevSummary: Summary | null) {
  ...
  const buildDetailRows = (transactionType: "revenue" | "expense") => {
    const mainMap = new Map<string, { amount: number; children: Map<string, number> }>();

    rows
      .filter((r) => r.transaction_type === transactionType)
      .forEach((r) => {
        const main = r.main_account_name || (transactionType === "revenue" ? "매출" : "비용");
        const child = r.detail_account_name || r.sub_account_name || main;
        const found = mainMap.get(main) || { amount: 0, children: new Map<string, number>() };

        found.amount += r.total_amount;
        found.children.set(child, (found.children.get(child) || 0) + r.total_amount);
        mainMap.set(main, found);
      });

    const output: DetailPnlRow[] = [];

    Array.from(mainMap.entries())
      .sort((a, b) => Math.abs(b[1].amount) - Math.abs(a[1].amount))
      .forEach(([main, value]) => {
        output.push({
          type: transactionType === "revenue" ? "매출" : "비용",
          level: "main",
          main,
          label: main,
          amount: value.amount,
          revenueRate: calcRate(value.amount, revenue),
          parentRate: null
        });

        Array.from(value.children.entries())
          ...
          .forEach(([label, amount]) => {
            ...
            output.push({
              ...
              label,
              amount,
              revenueRate: calcRate(amount, revenue),
              parentRate: value.amount ? calcRate(amount, value.amount) : 0
            });
          });
      });
  ...
  const detailedPnlRows = [
    ...buildDetailRows("revenue"),
    ...buildDetailRows("expense")
  ];
...
function getCashAccountKey(item: CashItem) {
  return item.account_number || `${item.group_name || "기타"}-${item.bank || ""}-${item.account_name || ""}`;
}

function getLatestAccountItems(items: CashItem[]) {
  const accountMap = new Map<string, CashItem>();

  items
    .filter((item) => item.item_type === "account")
    .forEach((item) => {
      const key = getCashAccountKey(item);
      const current = accountMap.get(key);
      const nextStamp = `${item.record_date || ""}-${item.id || ""}`;
      const currentStamp = current ? `${current.record_date || ""}-${current.id || ""}` : "";
...
function cashItemFromAccountGridRow(row: QuickCashRow): CashItem {
  return {
    id: row.id || row.row_key || row.account_number || row.account_name || "",
    record_date: row.record_date || getTodayDate(),
    settlement_month: toSettlementMonth(row.record_date || getTodayDate()),
    item_type: "account",
    group_name: row.group_name || "기타",
    category: row.category || "계좌",
...
function buildCashReport(items: CashItem[], todayDate: string) {
  ...
  // 통장 잔액은 같은 계좌가 여러 날짜에 저장될 수 있으므로, 계좌별 최신 1건만 합산합니다.
  const accounts = getLatestAccountItems(items);
...
export default function Page() {
  ...
  const [costInventoryRows, setCostInventoryRows] = useState<CostInventoryFormRow[]>(blankCostInventoryRows());
  const [costInventorySaving, setCostInventorySaving] = useState(false);
  ...
  const cashPrintRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    setForm((f) => {
      ...
      return {
        ...f,
        settlement_month: f.id ? f.settlement_month : toSettlementMonth(today),
        transaction_date: f.id ? f.transaction_date : today
      };
    });
    setCashForm((f) => ({
      ...f,
    ...
    setCostInventoryRows(blankCostInventoryRows());
  }, [month]);

  useEffect(() => {
    const nextAccounts = fixedCashAccounts.map((fixed) => {
      // 자금현황은 날짜별 기록입니다. 과거/최신 잔액을 끌고 오지 않고, 선택한 기준일(cashDate)의 저장값만 불러옵니다.
      const found = cashItems
        .filter((item) =>
          item.item_type === "account" &&
          item.record_date === cashDate &&
          ((fixed.account_number && item.account_number === fixed.account_number) || item.account_name === fixed.account_name)
        )
        .sort((a, b) => `${b.updated_at || ""}${b.created_at || ""}${b.record_date || ""}${b.id || ""}`.localeCompare(`${a.updated_at || ""}${a.created_at || ""}${a.record_date || ""}${a.id || ""}`))[0];
  ...
  useEffect(() => {
    if (!authed) return;
    if (tab !== "summary" && tab !== "analysis" && tab !== "franchise") return;
    refreshReportData();
  }, [authed, tab, month]);
  ...
  async function refreshAll() {
    await Promise.all([loadAccounts(), loadTransactions(), loadSummary(), loadPrevSummary(), loadCostInventory(), loadCashItems()]);
  }

  async function refreshReportData() {
    setReportRefreshing(true);
    try {
      await Promise.all([loadTransactions(), loadSummary(), loadPrevSummary(), loadCostInventory()]);
    } finally {
  ...
  async function loadAccounts() {
    const data = await api(`/api/accounts?_=${Date.now()}`);
    setAccounts(data.accounts || []);
  }

  async function loadTransactions() {
    const data = await api(`/api/transactions?month=${month}&_=${Date.now()}`);
    setTransactions(data.transactions || []);
  }

  async function loadSummary() {
    const data = await api(`/api/summary?month=${month}&_=${Date.now()}`);
    setSummary(data.summary);
  }

  async function loadPrevSummary() {
    const data = await api(`/api/summary?month=${getPrevMonth(month)}&_=${Date.now()}`);
    setPrevSummary(data.summary);
  }

  async function loadCostInventory() {
    try {
      const data = await api(`/api/cost-inventory?month=${month}&_=${Date.now()}`);
      const saved = data.items || [];
      const merged = blankCostInventoryRows().map((blank) => {
        const found = saved.find((item: any) => item.category === blank.category);

        return {
          ...blank,
          id: found?.id,
          settlement_month: found?.settlement_month || month,
          beginning_inventory: found?.beginning_inventory ? String(Number(found.beginning_inventory)) : "",
          ending_inventory: found?.ending_inventory ? String(Number(found.ending_inventory)) : "",
          memo: found?.memo || ""
        };
      });

      setCostInventoryRows(merged);
    } catch {
      setCostInventoryRows(blankCostInventoryRows());
    }
  }
  ...
  function updateCostInventoryRow(index: number, key: keyof CostInventoryFormRow, value: string) {
    setCostInventoryRows((prev) =>
      prev.map((row, rowIndex) => rowIndex === index ? { ...row, [key]: value } : row)
    );
  }

  async function saveCostInventoryRows() {
    setCostInventorySaving(true);
    try {
      const rows = costInventoryRows.map((row) => ({
        category: row.category,
        beginning_inventory: normalizeAmount(row.beginning_inventory),
        ending_inventory: normalizeAmount(row.ending_inventory),
        memo: row.memo || ""
      }));

      await api("/api/cost-inventory", {
        method: "POST",
        body: JSON.stringify({ month, rows })
      });

      await loadCostInventory();
      alert("기초/기말 재고를 저장했습니다.");
    } catch (e: any) {
      alert(e.message || "재고 저장 중 오류가 발생했습니다.");
    } finally {
      setCostInventorySaving(false);
    }
  }

  async function loadCashItems() {
    try {
      const data = await api(`/api/cash-items?month=${month}&cashDate=${cashDate}&dashboard=true&_=${Date.now()}`);
  ...
  async function generateCashPdf() {
    if (!cashPrintRef.current) {
      alert("PDF 미리보기 영역을 찾을 수 없습니다.");
      return;
    }
    ...
    try {
      ...
      const canvas = await html2canvas(cashPrintRef.current, {
        scale: 2,
      ...
      pdf.save(`palpal-cash-${cashDate}.pdf`);
    } catch (e: any) {
      alert(e.message || "PDF 저장 중 오류가 발생했습니다.");
    } finally {
      setPdfSaving(false);
    }
  }
  ...
  async function resetCashAccounts() {
    if (!confirm(`${cashDate} 기준 자금현황만 초기화할까요?\n\n이전 날짜 자금일보 기록은 유지됩니다. 출금완료/출금예정도 삭제되지 않습니다.`)) return;

    setBulkCashSaving(true);
    try {
      await api("/api/cash-items", {
        method: "DELETE",
        body: JSON.stringify({ item_type: "account", record_date: cashDate })
      });

      const resetRows = fixedCashAccounts.map((fixed) => ({
        ...fixed,
        id: "",
        record_date: cashDate,
        item_type: "account" as CashItemType,
      ...
      setAccountGridRows(resetRows);
      await loadCashItems();
      alert(`${cashDate} 기준 자금현황만 초기화했습니다. 이전 날짜 자금일보는 유지됩니다.`);
    } catch (e: any) {
      alert(e.message || "현재 기준일 자금현황 초기화 중 오류가 발생했습니다.");
    } finally {
  ...
  async function saveCashGridRows(rows: QuickCashRow[], itemType: CashItemType) {
    ...
    try {
      ...
      for (const [index, row] of rows.entries()) {
        ...
        const date = itemType === "account" ? cashDate : (row.record_date || (itemType === "out_done" ? cashDate : getTodayDate()));
        ...
        if (isAccount) {
          const payload = {
            id: row.id,
            record_date: date,
            settlement_month: toSettlementMonth(date),
            item_type: itemType,
            ...
            status: "",
            memo: row.memo,
            exclude_from_cash: row.exclude_from_cash
          };
        ...
        const blankNewRow = !row.id && !vendor && amount <= 0 && !memo;
        if (blankNewRow) continue;

        if (row.id && !vendor && amount <= 0) {
          await api("/api/cash-items", {
            method: "DELETE",
        ...
        if (!date) {
          invalidMessages.push(`${index + 1}행: 날짜를 입력하세요.`);
          continue;
        }

        if (!vendor) {
          invalidMessages.push(`${index + 1}행: 거래처를 입력하세요.`);
          continue;
        }

        if (amount <= 0) {
          invalidMessages.push(`${index + 1}행: 금액을 입력하세요.`);
          continue;
        }
        ...
        const payload = {
          ...
          settlement_month: toSettlementMonth(date),
          item_type: itemType,
          group_name: row.group_name || row.account_name || "팔팔",
          category: row.category || "식자재",
          vendor,
          amount,
          account_name: row.account_name || row.group_name || "팔팔",
          bank: row.bank,
          account_number: row.account_number,
          status: itemType === "out_done" ? "완료" : row.status || "예정",
          memo: row.memo,
          exclude_from_cash: false
        };
      ...
      if (invalidMessages.length) {
        alert(`저장하지 못한 행이 있습니다.\n\n${invalidMessages.slice(0, 6).join("\n")}${invalidMessages.length > 6 ? "\n..." : ""}`);
      }

      await loadCashItems();

      if (savedCount > 0 || deletedCount > 0) {
        alert(`${savedCount}건 저장${deletedCount ? `, ${deletedCount}건 삭제` : ""}했습니다.`);
      } else if (!invalidMessages.length) {
        alert("저장할 내용이 없습니다. 거래처와 금액을 입력하세요.");
      }
    } catch (e: any) {
      alert(e.message || "자금일보 저장 중 오류가 발생했습니다.");
    } finally {
      setBulkCashSaving(false);
    }
  }
  ...
  const salesProjection = useMemo(() => {
    return getMonthProjection(summary, month);
  }, [summary, month]);
  ...
  const costRateData = useMemo(() => {
    return buildCostRateData(summary, costInventoryRows);
  }, [summary, costInventoryRows]);

  const franchisePnlData = useMemo(() => {
    return buildFranchisePnlData(summary, costRateData);
  }, [summary, costRateData]);

  const currentAccountItems = useMemo(() => {
    return accountGridRows.map(cashItemFromAccountGridRow);
  }, [accountGridRows]);

  const accountGridCashTotal = useMemo(() => {
    // 최종 기준: 화면 아래 자금현황 표의 현재 잔액 - 합계제외 체크
    return currentAccountItems
      .filter((item) => !item.exclude_from_cash)
      .reduce((sum, item) => sum + Number(item.amount || 0), 0);
  }, [currentAccountItems]);

  const cashItemsForReport = useMemo(() => {
    // 과거 DB 계좌잔액은 완전히 제외하고, 현재 화면의 자금현황 표 4개 계좌만 사용합니다.
    const nonAccountItems = cashItems.filter((item) => item.item_type !== "account");
    return [...nonAccountItems, ...currentAccountItems];
  }, [cashItems, currentAccountItems]);

  const cashReport = useMemo(() => {
    const base = buildCashReport(cashItemsForReport, cashDate);

    return {
      ...base,
      accounts: currentAccountItems,
      totalCash: accountGridCashTotal,
      expectedBalance: accountGridCashTotal - base.scheduledTotal
    };
  }, [cashItemsForReport, currentAccountItems, accountGridCashTotal, cashDate]);
  ...
  return (
    <main className="container">
      ...
      <nav className="tabs no-print">
        ...
        <button className={`tab ${tab === "franchise" ? "active" : ""}`} onClick={() => setTab("franchise")}>🏪 가맹 손익 보고서</button>
      ...
      {tab === "summary" && (
        <section className="card report-card">
          ...
          <div className="report-section">
            ...
            <div className="report-comment light">
              계정별 금액과 매출 대비 비율, 그리고 상위 계정 안에서 차지하는 비율을 함께 봅니다. 월말재고는 손익에서 제외하고, 위 원가율 산정표에서 별도로 계산합니다.
            </div>

            <div className="report-table-wrap">
              <table className="report-table detail-pnl-table">
                <thead>
                  <tr>
                    <th>구분</th>
                    <th>계정</th>
                ...
                <tbody>
                  {reportData.detailedPnlRows.map((row, idx) => (
                    <tr key={`${row.type}-${row.main}-${row.label}-${idx}`} className={row.level === "main" ? "pnl-main-row" : "pnl-sub-row"}>
                      <td>{row.level === "main" ? row.type : ""}</td>
                      <td>{row.level === "main" ? row.label : `└ ${row.label}`}</td>
                      <td className={row.amount < 0 ? "num minus" : "num"}>{formatWon(row.amount)}</td>
                      <td className={row.revenueRate < 0 ? "num minus" : "num"}>{row.revenueRate}%</td>
                      <td className="num">{row.parentRate === null ? "-" : `${row.parentRate}%`}</td>
                    </tr>
                  ))}
                  <tr className="total-row">
                    <td>결과</td>
                    <td>순이익</td>
                    <td className={reportData.profit < 0 ? "num minus" : "num plus"}>{formatWon(reportData.profit)}</td>
                    <td className={reportData.profitRate < 0 ? "num minus" : "num plus"}>{reportData.profitRate}%</td>
                    <td className="num">-</td>
                  </tr>
                </tbody>
              </table>
      ...
      {tab === "franchise" && (
        <section className="card franchise-card">
          <div className="actions no-print">
            <button className="btn btn-ghost" onClick={refreshReportData} disabled={reportRefreshing}>
              {reportRefreshing ? "데이터 갱신 중..." : "데이터 새로고침"}
            </button>
            <button className="btn btn-ghost" onClick={() => window.print()}>가맹 보고서 PDF 출력</button>
          </div>

          <div className="franchise-hero">
            <div>
              <div className="franchise-eyebrow">FRANCHISE P&L REPORT</div>
              <h2>{month} 가맹 설명용 손익 보고서</h2>
              <p>가맹 희망자에게 보여줄 수 있도록 기초재고 + 매입 - 기말재고 기준의 실제 사용 원가로 손익을 재구성합니다.</p>
            </div>
            <div className="franchise-badge">상담용</div>
          </div>

          <div className="grid franchise-kpi-grid">
            <div className="franchise-kpi">
              <span>총매출</span>
              <strong>{formatWon(franchisePnlData.revenue)}</strong>
            </div>
            <div className="franchise-kpi">
              <span>재고반영 매출원가</span>
              <strong>{formatWon(franchisePnlData.adjustedCogs)}</strong>
              <small>{calcRate(franchisePnlData.adjustedCogs, franchisePnlData.revenue)}%</small>
            </div>
            <div className="franchise-kpi">
              <span>매출총이익</span>
              <strong>{formatWon(franchisePnlData.grossProfit)}</strong>
              <small>{franchisePnlData.grossProfitRate}%</small>
            </div>
            <div className="franchise-kpi">
              <span>영업이익</span>
              <strong className={franchisePnlData.operatingProfit >= 0 ? "profit" : "loss"}>{formatWon(franchisePnlData.operatingProfit)}</strong>
              <small>{franchisePnlData.operatingProfitRate}%</small>
            </div>
          </div>
          ...
          <div className="franchise-section">
            ...
            <div className="report-table-wrap">
              <table className="report-table franchise-table">
                <thead>
                  <tr>
                    <th>항목</th>
                    <th>금액</th>
                ...
                <tbody>
                  {franchisePnlData.pnlRows.map((row) => (
                    <tr key={row.label} className={`franchise-${row.type}`}>
                      <td>{row.label}</td>
                      <td className={row.amount < 0 ? "num minus" : "num"}>{formatWon(row.amount)}</td>
                      <td className="num">{row.rate}%</td>
                      <td>
                        {row.label === "매출원가" ? "기초재고 + 매입 - 기말재고" :
                         row.label === "매출총이익" ? "총매출 - 재고반영 매출원가" :
                         row.label === "영업이익" ? "매출총이익 - 운영비" : ""}
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="franchise-section">
            ...
            <div className="actions no-print cost-rate-actions">
              <button className="btn btn-green" onClick={saveCostInventoryRows} disabled={costInventorySaving}>
                {costInventorySaving ? "재고 저장 중..." : "기초/기말 재고 저장"}
              </button>
            </div>

            <div className="report-table-wrap">
              <table className="report-table cost-rate-table">
                <thead>
                  <tr>
                    <th>구분</th>
                    <th>기초재고</th>
                ...
                <tbody>
                  {costRateData.rows.map((row, idx) => (
                    <tr key={row.category}>
                      <td>{row.category}</td>
                      <td>
                        <input
                          className="report-cell-input"
                          value={costInventoryRows[idx]?.beginning_inventory ? Number(normalizeAmount(costInventoryRows[idx].beginning_inventory)).toLocaleString("ko-KR") : ""}
                          onChange={(e) => updateCostInventoryRow(idx, "beginning_inventory", e.target.value)}
                          placeholder="0"
                        />
                      </td>
                      <td className="num">{formatWon(row.purchase)}</td>
                      <td>
                        <input
                          className="report-cell-input"
                          value={costInventoryRows[idx]?.ending_inventory ? Number(normalizeAmount(costInventoryRows[idx].ending_inventory)).toLocaleString("ko-KR") : ""}
                          onChange={(e) => updateCostInventoryRow(idx, "ending_inventory", e.target.value)}
                          placeholder="0"
                        />
                      </td>
                      <td className="num">{formatWon(row.used)}</td>
                      <td className="num">{row.rate}%</td>
                    </tr>
                  ))}
                  <tr className="total-row">
                    <td>합계</td>
                    <td className="num">{formatWon(costRateData.total.beginning)}</td>
                    <td className="num">{formatWon(costRateData.total.purchase)}</td>
                    <td className="num">{formatWon(costRateData.total.ending)}</td>
                    <td className="num">{formatWon(costRateData.total.used)}</td>
                    <td className="num">{costRateData.total.rate}%</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="franchise-section">
            ...
            <div className="franchise-note-grid">
              <div>
                <strong>보여줄 숫자</strong>
                <p>월매출, 재고반영 원가율, 인건비율, 임차료율, 영업이익률 중심으로 설명합니다.</p>
              </div>
              <div>
                <strong>숨길 숫자</strong>
                <p>통장잔액, 출금예정, 미지급 거래처, 대표자 인출입, 부가세 납부 예정은 내부자료로 분리합니다.</p>
              </div>
              <div>
                <strong>회수기간 참고</strong>
                <p>영업이익 기준 1억 투자 회수기간은 약 {franchisePnlData.paybackBase || "-"}개월입니다.</p>
              </div>
            </div>
          </div>
        </section>
      ...
      {tab === "cash" && (
        <section className="card">
          <div className="cash-hero">
            ...
            <div className="actions no-print">
              <button className="btn btn-ghost" onClick={generateCashPdf} disabled={pdfSaving}>{pdfSaving ? "PDF 저장 중..." : "미리보기 PDF 저장"}</button>
            </div>
          </div>
          ...
          <div className="cash-pdf-preview-section">
            <div className="cash-panel-head no-print">
              <h3>PDF 미리보기</h3>
              <span>아래 화면 그대로 모바일/PC에서 PDF 저장됩니다</span>
            </div>
            <div className="cash-pdf-preview-scroll">
              <div className="cash-print-report" ref={cashPrintRef}>
          ...
          <div className="cash-grid-section">
            <div className="cash-panel-head">
              <h3>자금현황</h3>
              <span>통장 합계/PDF는 선택 기준일의 자금현황 4개 기준</span>
            </div>
            ...
            <div className="cash-checksum no-print">
              <span>통장 합계 검산</span>
              <strong>{formatWon(accountGridCashTotal)}</strong>
              <em>위 현재 보유자금과 동일해야 정상입니다.</em>
            </div>
                ...
                <tbody>
                  {todayOutGridRows.map((row, index) => (
                    <tr key={row.row_key}>
                      ...
                      <td><input className="cell-input" list="vendor-suggestions" value={row.vendor} onChange={(e) => updateGridRow(setTodayOutGridRows, index, "vendor", e.target.value)} placeholder="거래처" /></td>
                ...
                <tbody>
                  {scheduledGridRows.map((row, index) => (
                    <tr key={row.row_key}>
                      ...
                      <td><input className="cell-input" list="vendor-suggestions" value={row.vendor} onChange={(e) => updateGridRow(setScheduledGridRows, index, "vendor", e.target.value)} placeholder="거래처" /></td>
      ...
      {tab === "analysis" && (
        <section className="card">
          <div className="actions no-print">
            <button className="btn btn-ghost" onClick={() => window.print()}>분석표 PDF 출력</button>
          </div>
          ...
          <div className="sales-projection-card">
            <div className="sales-projection-head">
              <div>
                <div className="sales-projection-label">MONTHLY SALES FORECAST</div>
                <h3>월 매출 예상</h3>
              </div>
              <span>{salesProjection.status}</span>
            </div>

            <div className="sales-projection-grid">
              <div>
                <small>현재 누적매출</small>
                <strong>{formatWon(salesProjection.revenue)}</strong>
              </div>
              <div>
                <small>현재 일평균</small>
                <strong>{formatWon(salesProjection.dailyRevenue)}</strong>
              </div>
              <div>
                <small>월매출 예상</small>
                <strong>{formatWon(salesProjection.projectedRevenue)}</strong>
              </div>
              <div>
                <small>예상 순이익</small>
                <strong>{formatWon(salesProjection.projectedProfit)}</strong>
              </div>
            </div>

            <div className="sales-projection-comment">
              {salesProjection.comment}
              {salesProjection.isCurrentMonth && salesProjection.remainingDays > 0 && (
                <>
                  <br />
                  6천만 원을 맞추려면 남은 {salesProjection.remainingDays}일 동안 하루 평균 {formatWon(salesProjection.requiredDailyTargetLow)},
                  6천5백만 원을 맞추려면 하루 평균 {formatWon(salesProjection.requiredDailyTargetHigh)}가 필요합니다.
                </>
              )}
            </div>
          </div>
