...
export default function Page() {
  ...
  const cashItemsForReport = useMemo(() => {
    // 자금현황 합계/PDF는 DB에 남아있는 과거 계좌잔액이 아니라, 화면의 현재 자금현황 표 기준으로 계산합니다.
    const nonAccountItems = cashItems.filter((item) => item.item_type !== "account");
    const currentAccounts = accountGridRows.map(cashItemFromAccountGridRow);
    return [...nonAccountItems, ...currentAccounts];
  }, [cashItems, accountGridRows]);

  const cashReport = useMemo(() => {
    return buildCashReport(cashItemsForReport, cashDate);
  }, [cashItemsForReport, cashDate]);
