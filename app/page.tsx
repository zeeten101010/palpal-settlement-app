...
export default function Page() {
  ...
  useEffect(() => {
    const nextAccounts = fixedCashAccounts.map((fixed) => {
      const found = getLatestAccountItems(cashItems)
        .find((item) =>
      ...
      return {
        ...fixed,
        id: found?.id || "",
        record_date: found?.record_date || getTodayDate(),
  ...
  async function saveCostInventoryRows() {
    ...
    try {
      await api("/api/cost-inventory", {
        ...
        body: JSON.stringify({
          month,
          rows: costInventoryRows.map((row) => ({
            ...row,
            settlement_month: month,
  ...
  async function generateCashPdf() {
    if (!cashPrintRef.current) return;
    ...
    try {
      ...
      const target = cashPrintRef.current;
      ...
      const rawImgHeight = (canvas.height * usableWidth) / canvas.width;
      const imgHeight = Math.min(rawImgHeight, usableHeight);
      const imgData = canvas.toDataURL("image/jpeg", 0.96);

      pdf.addImage(imgData, "JPEG", marginX, marginY, usableWidth, imgHeight);
      pdf.save(`자금일보_${cashDate}.pdf`);
      alert("PDF 저장 요청이 완료되었습니다. 모바일에서는 다운로드 폴더 또는 파일 앱에서 확인하세요.");
    } catch (e: any) {
  ...
  async function saveCashGridRows(rows: QuickCashRow[], itemType: CashItemType) {
    ...
    try {
      ...
      for (const [index, row] of rows.entries()) {
        ...
        const date = row.record_date || (itemType === "out_done" ? cashDate : getTodayDate());
        ...
        <section className="card">
          ...
          <div className="cash-grid-section">
            <div className="cash-panel-head">
              <h3>자금현황</h3>
              <span>통장 합계/PDF는 아래 표의 현재 잔액 기준</span>
            </div>
